...
FilenameAttribute::FilenameAttribute(const std::string           &label,
    ...
    : AbstractAttribute(AttributeType::FILENAME, label), value(value),
      for_saving(for_saving), directory_only(directory_only), filter(filter)
{
  this->save_state();
...
bool FilenameAttribute::get_directory_only() const { return this->directory_only; };
...
void FilenameAttribute::json_from(nlohmann::json const &json)
{
  ...
  json_safe_get(json, "directory_only", directory_only);
...
nlohmann::json FilenameAttribute::json_to() const
{
  ...
  json["directory_only"] = this->directory_only;
