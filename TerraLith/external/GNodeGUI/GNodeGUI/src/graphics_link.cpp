                         ...
                         QWidget                        *widget)
{
  ...
  QColor pcolor = this->is_drop_target ? QColor(100, 200, 255)
                  : this->isSelected() ? GN_STYLE->link.color_selected
                                       : this->color;
  float  pwidth = this->is_drop_target ? GN_STYLE->link.pen_width_hovered * 1.5f
                  : this->is_link_hovered
  ...
  // --- Success flash: bright glow overlay that fades out
  if (this->flash_alpha_ > 0.01f)
  {
    QColor flash_color = Qt::white;
    flash_color.setAlphaF(this->flash_alpha_ * 0.6f);

    QPen flash_pen(flash_color);
    flash_pen.setWidthF(pwidth + 4.0f * this->flash_alpha_);
...
void GraphicsLink::set_flash_alpha(float alpha)
{
  this->flash_alpha_ = alpha;
  this->update();
}

void GraphicsLink::set_is_drop_target(bool state)
{
  if (this->is_drop_target != state)
  {
    this->is_drop_target = state;
    this->update();
  }
}
