...
public:
  ...
  /**
   * @brief Get the value at (x, y) using bicubic (Catmull-Rom) interpolation
   * across tiles.  Provides C1-continuous derivatives for smooth surface
   * normals when upscaling heightmaps.
   *
   * @param x  Horizontal coordinate in [0, 1).
   * @param y  Vertical   coordinate in [0, 1).
   * @return   float  Bicubic-interpolated value.
   */
  float get_value_bicubic(float x, float y) const;
