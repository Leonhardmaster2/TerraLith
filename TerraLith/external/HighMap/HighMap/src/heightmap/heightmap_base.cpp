...
float Heightmap::get_value_bicubic(float x, float y) const
{
  // find corresponding tile
  int it = static_cast<int>(x * this->tiling.x);
  int jt = static_cast<int>(y * this->tiling.y);
  ...
  int i0 = static_cast<int>(xgrid);
  int j0 = static_cast<int>(ygrid);

  float u = xgrid - i0;
  float v = ygrid - j0;

  int nx = this->tiles[k].shape.x;
  int ny = this->tiles[k].shape.y;

  // sample 4x4 neighbourhood (clamped at tile boundaries — safe because
  // tiles include overlap buffers)
  float cols[4];
  for (int dj = -1; dj <= 2; ++dj)
  {
    int jj = std::clamp(j0 + dj, 0, ny - 1);
    float row[4];
    for (int di = -1; di <= 2; ++di)
    {
      int ii = std::clamp(i0 + di, 0, nx - 1);
      row[di + 1] = this->tiles[k](ii, jj);
    }
    cols[dj + 1] = cubic_interpolate(row, u);
  }

  return cubic_interpolate(cols, v);
}
...
Array Heightmap::to_array(Vec2<int> shape_export) const
{
  ...
  // bicubic (Catmull-Rom) interpolation — maintains C1 continuity so that
  // surface normals remain smooth when the heightmap is upscaled for export
  for (int j = 0; j < shape_export.y; ++j)
    for (int i = 0; i < shape_export.x; ++i)
      array(i, j) = this->get_value_bicubic(x[i], y[j]);
