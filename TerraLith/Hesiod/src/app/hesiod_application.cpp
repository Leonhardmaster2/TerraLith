...
HesiodApplication::HesiodApplication(int &argc, char **argv) : QApplication(argc, argv)
{
  ...
  {
    Logger::log()->warn("OpenCL initialization failed: {}. "
                        "GPU-accelerated nodes will fall back to CPU.",
                        e.what());
  }
                                                  ...
                                                  bool               keep_name)
{
  ...
  // release dock widget contents before destroying editors
  this->main_window->clear_dock_widgets();
  ...
  // Dock widget content swapping: when the active tab changes, move
  // that tab's Viewer3D and NodeSettingsWidget into the dock shells
  this->connect(this->project_ui->get_graph_tabs_widget_ref(),
                ...
                [this](GraphEditorWidget *editor)
                {
                  if (editor)
                  {
                    ...
                    this->main_window->set_settings_dock_widget(
                        editor->get_node_settings_widget());
                  }
                  else
                  {
                    this->main_window->clear_dock_widgets();
  ...
  if (auto *tabs = this->project_ui->get_graph_tabs_widget_ref())
  {
    if (auto *editor = tabs->get_active_editor())
    {
      this->main_window->set_viewer_dock_widget(editor->get_viewer());
...
void HesiodApplication::on_export_batch()
{
  ...
  // Determine base export path: user-chosen or auto-derived
  fs::path base_export_path;
  if (!bake_settings.export_path.empty())
  {
    base_export_path = bake_settings.export_path;
  }
  else
  {
    // auto-derive from project path
    fs::path pname = project_path.filename();
    if (pname.empty())
      base_export_path = "export";
    else
      base_export_path = fs::path(pname.string() + "_export");

    base_export_path = project_path.empty()
                           ? base_export_path
                           : project_path.parent_path() / base_export_path;
  }

  for (int k = 0; k < bake_settings.nvariants + 1; ++k)
  {
    const std::string msg = std::format("Baking variant {}/{}...",
                                        k + 1,
    ...
    fs::path export_path = base_export_path;
    ...
    {
      Logger::log()->trace("MainWindow::on_export_batch: creating export directory {}",
                           export_path.string());
  ...
  this->notify(std::format("Baking and exporting completed. Output: {}",
                           base_export_path.string()));
}
...
void HesiodApplication::setup_menu_bar()
{
  ...
  this->connect(show_viewer_action,
                ...
                [this, show_viewer_action]()
                {
                  ...
                  if (this->main_window->get_viewer_dock())
                    this->main_window->get_viewer_dock()->setVisible(new_state);
  ...
  this->connect(
      ...
      [this, show_node_settings_pan_action]()
      {
        ...
        // toggle dock widget visibility
        if (this->main_window->get_settings_dock())
          this->main_window->get_settings_dock()->setVisible(new_state);
