...
void GraphManager::change_resolution(int new_resolution)
{
  Logger::log()->trace("GraphManager::change_resolution: {}", new_resolution);

  // Build a new config based on the first graph's current config (all graphs
  // share the same shape/tiling/overlap values) and only change the shape.
  for (auto &graph_id : this->graph_order)
  {
    GraphNode *p_graph = this->graph_nodes.at(graph_id).get();
    GraphConfig new_config = *p_graph->get_config_ref();
    new_config.set_shape(hmap::Vec2<int>(new_resolution, new_resolution));
    p_graph->change_config_values(new_config);
  }
}
