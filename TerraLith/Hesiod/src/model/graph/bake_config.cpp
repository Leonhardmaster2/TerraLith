...
void BakeConfig::json_from(nlohmann::json const &json)
{
  ...
  json_safe_get(json, "format_override", format_override);

  if (json.contains("export_path"))
  {
    std::string path_str;
    json_safe_get(json, "export_path", path_str);
    export_path = path_str;
  }

  if (json.contains("base_name"))
    json_safe_get(json, "base_name", base_name);
}
...
nlohmann::json BakeConfig::json_to() const
{
  ...
  json["format_override"] = format_override;
  json["export_path"] = export_path.string();
  json["base_name"] = base_name;
  return json;
}
