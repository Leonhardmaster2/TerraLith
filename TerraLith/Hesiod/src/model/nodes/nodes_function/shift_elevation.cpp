...
#ifdef HESIOD_HAS_VULKAN
bool compute_shift_elevation_node_vulkan(BaseNode &node)
{
  hmap::Heightmap *p_in = node.get_value_ref<hmap::Heightmap>("input");
  ...
  hmap::Heightmap *p_out = node.get_value_ref<hmap::Heightmap>("output");
  float shift = node.get_attr<FloatAttribute>("shift");

  struct { uint32_t width; uint32_t height; float shift; } pc{};
  ...
  for (size_t i = 0; i < p_out->get_ntiles(); ++i)
  {
    ...
    gp.dispatch("shift_elevation", &pc, sizeof(pc), buffers, (pc.width + 15) / 16, (pc.height + 15) / 16);
