...
void setup_combiner_divide_node(BaseNode &node)
{
...
void compute_combiner_divide_node(BaseNode &node)
{
  ...
  if (p_in1 && p_in2)
  {
    ...
    hmap::transform(
        ...
        [](std::vector<hmap::Array *> p_arrays)
        {
          ...
          for (int k = 0; k < pa_out->size(); ++k)
          {
            float divisor = (*pa_in2).vector[k];
            (*pa_out).vector[k] = (std::abs(divisor) > 1e-10f)
                                      ? (*pa_in1).vector[k] / divisor
                                      : 0.f;
          }
        },
...
#ifdef HESIOD_HAS_VULKAN
bool compute_combiner_divide_node_vulkan(BaseNode &node)
{
  hmap::Heightmap *p_in1 = node.get_value_ref<hmap::Heightmap>("input 1");
  hmap::Heightmap *p_in2 = node.get_value_ref<hmap::Heightmap>("input 2");
  ...
  for (size_t i = 0; i < p_out->get_ntiles(); ++i)
  {
    ...
    gp.dispatch("combiner_divide", &pc, sizeof(pc), buffers, (pc.width + 15) / 16, (pc.height + 15) / 16);
