...
void setup_combiner_multiply_node(BaseNode &node)
{
...
void compute_combiner_multiply_node(BaseNode &node)
{
...
#ifdef HESIOD_HAS_VULKAN
bool compute_combiner_multiply_node_vulkan(BaseNode &node)
{
  hmap::Heightmap *p_in1 = node.get_value_ref<hmap::Heightmap>("input 1");
  hmap::Heightmap *p_in2 = node.get_value_ref<hmap::Heightmap>("input 2");
  ...
  for (size_t i = 0; i < p_out->get_ntiles(); ++i)
  {
    ...
    gp.dispatch("combiner_multiply", &pc, sizeof(pc), buffers, (pc.width + 15) / 16, (pc.height + 15) / 16);
