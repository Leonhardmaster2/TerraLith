...
void Viewer::json_from(nlohmann::json const &json)
{
  ...
  // Only restore widget geometry when not embedded in a dock widget.
  // When docked, the dock's layout manager controls the geometry.
  if (this->isWindow())
  {
    int x = json["x"];
    int y = json["y"];
...
void Viewer::setup_layout()
{
  ...
  layout->setSpacing(0);
  layout->setColumnStretch(0, 1); // renderer column fills all available space
